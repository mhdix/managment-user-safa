...
function AddParticipantModal({
  ...
  const handleAdd = async () => {
    ...
    await supabase
      ...
      .insert(
        selected.map((person_id) => ({ program_id: programId, person_id })),
      );
    setSaving(false);
    onAdded();
    onClose();
      ...
      footer={
        <>
          ...
          >
            {saving
              ? "در حال افزودن..."
              : `افزودن ${selected.length ? `(${selected.length} نفر)` : ""}`}
          </button>
    ...
    >
      {people.length === 0 ? (
        <p className="text-muted">
          همه افراد قبلاً اضافه شده‌اند یا هیچ فردی ثبت نشده است.
        </p>
      ) : (
        <div
          style={{ display: "grid", gap: 8, maxHeight: 360, overflowY: "auto" }}
        >
          {people.map((p) => {
  ...
  return (
    <div>
        ...
        <div className="flex-between">
          <div>
            ...
            <p className="text-muted text-sm mt-4">
              📅 {toJalali(program.date)} &nbsp;|&nbsp; ⏰ {program.start_time}
              {program.end_time ? ` تا ${program.end_time}` : ""}
            </p>
            {program.description && (
      ...
      {removeId && (
          ...
          footer={
            <>
              <button
                ...
                onClick={() => removeParticipant(removeId)}
              >
                بله، حذف شود
              </button>
              <button
        ...
        >
          <p>
            این فرد از لیست شرکت‌کنندگان حذف می‌شود. رکورد حضور او نیز در صورت
            وجود باقی می‌ماند.
          </p>
        </Modal>
      )}
